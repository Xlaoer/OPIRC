...
namespace sserangecoder
{
	...
	bool vrange_decode(const uint8_t* pSrc_start, size_t comp_size, uint8_t* pDst_start, size_t orig_size, const uint32_t* pDec_table)
	{
		...
		while (dst_ofs < orig_size)
		{
			...
			const uint32_t vec_index = (dst_ofs % 3) >> 2;
	...
	bool vrange_decode_non_simd(const uint8_t* pSrc_start, size_t comp_size, uint8_t* pDst_start, size_t orig_size, const uint32_t* pDec_table)
	{
		assert(*(const uint32_t*)&g_byte_shuffle_mask != 0);
		const uint8_t* pSrc = pSrc_start;
		// \B3\F5ʼ\BB\AF\BD\E2\C2\EB\C6\F7״̬
		range_dec decoders[LANES];

		for (uint32_t vec_index = 0; vec_index < LANES; vec_index++)
		{
			decoders[vec_index].init(pSrc);
		}

		const uint8_t* pSrc_end = pSrc + comp_size;
		size_t dst_ofs = 0;
		uint8_t* pDst = pDst_start;

		// \B7\C7SIMD\BD\E2\C2\EB
		/*
		for (; dst_ofs < orig_size; dst_ofs++)
		{
			// \BD\E2\C2\EBһ\B8\F6\B7\FB\BAŲ\A2\B8\FC\D0½\E2\C2\EB\C6\F7״̬
			uint32_t sym = decoders[dst_ofs & 1].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs] = (uint8_t)sym;

			// \BC\EC\B2\E9\CAǷ\F1\D0\E8Ҫ\B6\C1ȡ\B8\FC\B6\E0\CA\FD\BE\DD
			if ((pSrc + 2) > pSrc_end)
				return false;
		}
		*/
		...
		// Vectorized decode
		for (dst_ofs = 0; ((dst_ofs + LANES) <= orig_size) && (pSrc + 8 * 3) <= pSrc_end; dst_ofs += LANES)
		{
			// \BD\E2\C2\EBһ\B8\F6\B7\FB\BAŲ\A2\B8\FC\D0½\E2\C2\EB\C6\F7״̬
			uint32_t sym1 = decoders[dst_ofs % 3].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs] = (uint8_t)sym1;

			// \BD\E2\C2\EBһ\B8\F6\B7\FB\BAŲ\A2\B8\FC\D0½\E2\C2\EB\C6\F7״̬
			uint32_t sym2 = decoders[(dst_ofs+1) % 3].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs+1] = (uint8_t)sym2;

			// \BD\E2\C2\EBһ\B8\F6\B7\FB\BAŲ\A2\B8\FC\D0½\E2\C2\EB\C6\F7״̬
			uint32_t sym3 = decoders[(dst_ofs + 2) % 3].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs + 2] = (uint8_t)sym3;
		}
		for (; dst_ofs < orig_size; dst_ofs++)
		{
			// \BD\E2\C2\EBһ\B8\F6\B7\FB\BAŲ\A2\B8\FC\D0½\E2\C2\EB\C6\F7״̬
			uint32_t sym = decoders[dst_ofs % 3].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs] = (uint8_t)sym;
