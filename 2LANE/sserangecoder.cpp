...
namespace sserangecoder
{
	...
	bool vrange_decode_non_simd(const uint8_t* pSrc_start, size_t comp_size, uint8_t* pDst_start, size_t orig_size, const uint32_t* pDec_table)
	{
		...
		{
			//     һ     Ų    ½     ״̬
			uint32_t sym = decoders[dst_ofs & LANE_MASK].dec_sym(pDec_table, pSrc);
		...
		*/
		...
		// Vectorized decode
		for (dst_ofs = 0; ((dst_ofs + LANES) <= orig_size) && (pSrc + 8 * 2) <= pSrc_end; dst_ofs += LANES)
		{
			//     һ     Ų    ½     ״̬
			uint32_t sym1 = decoders[dst_ofs & LANE_MASK].dec_sym(pDec_table, pSrc);
			pDst[dst_ofs] = (uint8_t)sym1;

			//     һ     Ų    ½     ״̬
			uint32_t sym2 = decoders[(dst_ofs+1) & LANE_MASK].dec_sym(pDec_table, pSrc);
