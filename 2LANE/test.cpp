	...
	double total_theoretical_bits)
{
	...
	printf("plain Compressed file from %zu bytes to %zu bytes, %.3f%% vs. theoretical limit\n",
	...
	double total_theoretical_bits)
{
	...
	printf("2LANE encoding %.1f MiB/sec.\n", ((double)file_size / total_enc_time) / (1024 * 1024));
	...
	for (uint32_t r = 0; r < OUTER_TIMES; r++)
	{
		...
		for (uint32_t times = 0; times < TIMES_TO_DECODE; times++)
		{
			...
			if (!vrange_decode_non_simd(&enc_buf[0], enc_buf.size(), &decoded_buf[0], file_size, &dec_table[0]))
				panic("range_decode() failed!\n");
		...
		if (memcmp(&decoded_buf[0], &file_data[0], file_size) != 0)
			panic("Decompression failed!\n");
		printf("2LANE Compressed file from %zu bytes to %zu bytes\n",
			file_data.size(), enc_buf.size());

		printf("2LANE decoding %.1f MiB/sec.\n", ((double)file_size / total_time) / (1024 * 1024));
		printf("Decompression OK\n");

		//printf("2%.6f seconds, %.1f MiB/sec., %.1f cycles per byte\n", total_time, ((double)file_size / total_time) / (1024*1024), ((double)total_cycles / TIMES_TO_DECODE) / file_size);
	} // r
}
...
static bool interleaved_encode(const uint8_vec& file_data, uint8_vec& comp_data, uint32_t& file_data_crc32)
{
	const uint32_t file_size = (uint32_t)file_data.size();
	if ((!file_size) || (file_size > UINT32_MAX))
		return false;

	file_data_crc32 = crc32(0, &file_data[0], file_size);
...
int main(int argc, char **argv)
{
	...
	else 
	{
		...
		if (mode == cModeComp)
		{
			...
			status = interleaved_encode(file_data, out_data, expected_crc32);
