...
int main(int argc, char** argv)
{
   ...
   //printf("\n------------Output-Interleaved Encoding starts!--------------\n");
	...
	range_enc encs0, encs1;//定义2个编码器
	encs0.get_buf().reserve(1 + (file_size / 2));//为每个编码器的输出缓冲分配空间
	encs1.get_buf().reserve(1 + (file_size / 2));

	int CurBlockIndex = 0;//窗口的起始块索引

	int e0 = -2, e1 = -1;//编码器存放输出的起始位置
	...
	while (i < index) {
		...
		if (e0 < CurBlockIndex || e1 < CurBlockIndex )
			continue;
		CurBlockIndex += 2;
	}

	// Encode the last 2 symbols
	while (i < file_size) {
		//run encs0
		if (e0 < CurBlockIndex) {
			sym0 = file_data[i++];
	...
	printf("OPIRC-2way encoding %.1f MiB/sec.\n", 
	...
	while (i < index) {
		...
		if (d0 < CurBlockIndex || d1 < CurBlockIndex )
			continue;
		CurBlockIndex += 2;
	}

	// Decode the last 2 symbols
	while (i < file_size) {
		//run dec0
		if (d0 < CurBlockIndex) {
			decoded_symbol[i++] = (uint8_t)dec0.dec_sym_interleave(&dec_table[0], d0);
	...
	printf("OPIRC-2way decoding  %.1f MiB/sec.\n", ((double)file_size / total_dec_time) / (1024 * 1024));

	printf("OPIRC-2way Compressed file from %zu bytes to %zu bytes\n",
